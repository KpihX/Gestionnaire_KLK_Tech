...
def frameVerticalScrollbar(window:Tk, canvas:Canvas, frame):
...
def inputWindow(window:Tk, parent, Str:str):
...
def leaveWindow(window:Tk, parent, text:str = 'Quitter'):
...
def nextButton(window:Tk, parent, text:str = '⇩'):
...
def openJsonFile(Datas, path, mode='r'):
    """It takes as input the path of a json file, the mode of access to this file ('r' or 'w') and a data structure 'Datas'
    -If mode='r' it puts the contents of this file in 'Datas'
    -If mode='w', it copies the contents of 'Datas' in this file
    """
    with open(path+'.json', mode) as f:
        if mode == 'r':
            extend(Datas, json.load(f))
        if mode == 'w':
...
def printTableWindows(parent, Heading, Datas):
...
def printWindow(parent, Str:str):
...
if __name__ == "__main__":
    import Manager_Functions_GUI_2
